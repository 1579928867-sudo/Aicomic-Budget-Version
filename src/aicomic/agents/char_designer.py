...
5. **Full prompt**: Generate a complete Chinese image-generation prompt (full_prompt) for each variant. This prompt must follow the format:
   - For characters in Chinese ancient settings: Start with "古代仙侠风格，" then "【时代背景】角色名（代称），性别 年龄，身高cm，九头身比例，写实电影感风格，正面，站立的全身图片，图片人物背景为纯白色。发色发型；头饰；妆造脸部特征气质；上身描述；下身描述；脚上描述；配饰；双手自然下垂，手里无任何物品。"
...
8. **Cinematic realistic style** (写实电影感风格) for all characters.
9. **For animals/beasts**: Different format — species, age, body length/height, fur/skin color/texture, facial features, distinctive decorations. Format: "【时代背景】写实电影感风格，正面，全貌图片，图片背景为纯白色。角色名（代称）｜物种｜年龄｜体长/身高｜皮毛描述｜面部特征｜装饰"
